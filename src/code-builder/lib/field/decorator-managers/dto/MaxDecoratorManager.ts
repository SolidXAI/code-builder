...
export interface MaxDecoratorOptions {
  isApplyMax: boolean;
  max: number;
  source: any;
  field: any;
...
export class MaxDecoratorManager implements DecoratorManager {
  constructor(public options: MaxDecoratorOptions, public fieldNode?: any) { }
  ...
  createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
    ...
    if (existingDecorator !== undefined) {
      ...
      if (existingCallExpression.arguments.length > 1) {
        if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[1])) { throw new Error('Max decorator 2nd argument must be an object literal containing the validation options'); }
