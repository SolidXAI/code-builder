...
export interface TransformDecoratorOptions {
  isTransform: boolean;
  type: string;
  source: ts.SourceFile;
  field: any;
}
export class TransformDecoratorManager implements DecoratorManager { //FIXME : This decorator needs to be replaced with the IsTransform decorator
  constructor(public options: TransformDecoratorOptions, public fieldNode?: any) { }
  ...
  createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
    ...
    if (existingDecorator !== undefined) {
      ...
      if (existingCallExpression.arguments.length > 1) {
        if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[1])) { throw new Error('Transform decorator 1st argument must be an object literal containing the validation options'); }
    ...
    const transformFunction = ts.factory.createArrowFunction(
      ...
      ts.factory.createIdentifier(this.options.type)
    );
