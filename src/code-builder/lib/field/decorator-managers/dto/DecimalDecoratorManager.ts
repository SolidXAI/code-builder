...
export interface DecimalDecoratorOptions {
  isDecimal: boolean;
  source: ts.SourceFile;
  field: any;
}
export class DecimalDecoratorManager implements DecoratorManager {
  constructor(public options: DecimalDecoratorOptions, public fieldNode?: any) { }
  isApplyDecorator(): boolean {
    return this.options.isDecimal;
  }
  ...
  createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
    ...
    const decimalOptions: ts.ObjectLiteralElementLike[] = [];
    ...
    if (existingDecorator !== undefined) {
      ...
      if (existingCallExpression.arguments.length > 0) {
        if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[0])) { throw new Error('Decimal decorator 2nd argument must be an object literal containing the validation options'); }
        const existingObjectLiteralExpression = existingCallExpression.arguments[0] as ts.ObjectLiteralExpression;
        decimalOptions.push(...existingObjectLiteralExpression.properties);
        if (existingCallExpression.arguments.length > 1) {
          if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[1])) { throw new Error('Decimal decorator 3rd argument must be an object literal containing the validation options'); }
    ...
    if (decimalOptions.length > 0) {
      const decimalOptionsObjectLiteral = ts.factory.createObjectLiteralExpression(decimalOptions);
      argumentsArray.push(decimalOptionsObjectLiteral);
    }
