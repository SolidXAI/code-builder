...
export interface BigIntDecoratorOptions {
  isBigInt: boolean;
  source: ts.SourceFile;
  field: any;
}
export class BigIntDecoratorManager implements DecoratorManager {
  constructor(public options: BigIntDecoratorOptions, public fieldNode?: any) { }
  ...
  createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
    ...
    if (existingDecorator !== undefined) {
      ...
      if (existingCallExpression.arguments.length > 0) {
        if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[0])) { throw new Error(`${this.decoratorName()} decorator 1rd argument must be an object literal containing the validation options`); }
