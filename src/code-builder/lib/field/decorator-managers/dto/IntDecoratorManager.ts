import ts, { ModifierLike } from "@schematics/angular/third_party/github.com/Microsoft/TypeScript/lib/typescript";
import { DecoratorManager, PartialAddFieldChange } from "../../FieldManager";
import { insertImport } from "@schematics/angular/utility/ast-utils";
import { Change } from "@schematics/angular/utility/change";

export interface IntDecoratorOptions {
  isInt: boolean;
  source: ts.SourceFile;
  field: any;
}
export class IntDecoratorManager implements DecoratorManager { //FIXME : This decorator needs to be replaced with the IsInt decorator
  constructor(public options: IntDecoratorOptions, public fieldNode?: any) { }
  isApplyDecorator(): boolean {
    return this.options.isInt;
  }
  ...
  createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
    ...
    if (existingDecorator !== undefined) {
      ...
      if (existingCallExpression.arguments.length > 0) {
        if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[0])) { throw new Error('Int decorator 1st argument must be an object literal containing the validation options'); }
