...
export class BooleanDecoratorManager implements DecoratorManager { //FIXME : This decorator needs to be replaced with the isBoolean decorator
  constructor(public options: BooleanDecoratorOptions, public fieldNode?: any) { }
  ...
  createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
    ...
    if (existingDecorator !== undefined) {
      ...
      if (existingCallExpression.arguments.length > 0) {
        if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[0])) { throw new Error('Boolean decorator 1st argument must be an object literal containing the validation options'); }
