...
export class DateDecoratorManager implements DecoratorManager { //FIXME : This decorator needs to be replaced with the IsDate decorator
  constructor(public options: DateDecoratorOptions, public fieldNode?: any) { }
  ...
  createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
    ...
    if (existingDecorator !== undefined) {
      ...
      if (existingCallExpression.arguments.length > 0) {
        if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[0])) { throw new Error('Date decorator 1st argument must be an object literal containing the validation options'); }
    ...
    const decoratorIdentifier = ts.factory.createIdentifier("IsDate");
