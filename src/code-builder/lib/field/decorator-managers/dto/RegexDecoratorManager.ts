...
export class RegexDecoratorManager implements DecoratorManager {
  constructor(public options: RegexDecoratorOptions, public fieldNode?: PropertyDeclaration | undefined) { }
  ...
  createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
    ...
    const decoratorIdentifier = ts.factory.createIdentifier("Matches");
    const argumentsArray: ts.Expression[] = [];
    const regexPattern = ts.factory.createRegularExpressionLiteral(`/${this.options.regexPattern}/`);
