...
export interface OptionalDecoratorOptions {
    isApplyOptional: boolean;
    optional: boolean;
    source: ts.SourceFile;
    field: any;
}
export class OptionalDecoratorManager implements DecoratorManager {
    constructor(public options: OptionalDecoratorOptions, public fieldNode?: any) { }
    ...
    createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
        ...
        let existingValidationOptions: ts.ObjectLiteralElementLike[] = this.existingDecoratorOptions(existingDecorator);
        ...
        if (existingValidationOptions.length > 0) {
            const validationOptionsObjectLiteral = ts.factory.createObjectLiteralExpression(existingValidationOptions);
    ...
    private existingDecoratorOptions(existingDecorator: ts.Decorator | undefined) {
        ...
        if (existingDecorator !== undefined) {
            ...
            if (existingCallExpression.arguments.length > 0) {
                if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[0])) { throw new Error('IsOptional decorator 1st argument must be an object literal containing the validation options'); }
