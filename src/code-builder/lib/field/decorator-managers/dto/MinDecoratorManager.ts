...
export interface MinDecoratorOptions {
  isApplyMin: boolean;
  min: number;
  source: any;
  field: any;
}
export class MinDecoratorManager implements DecoratorManager {
  constructor(public options: MinDecoratorOptions, public fieldNode?: any) { }
  ...
  createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
    ...
    if (existingDecorator !== undefined) {
      ...
      if (existingCallExpression.arguments.length > 1) {
        if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[1])) { throw new Error('Min decorator 2nd argument must be an object literal containing the validation options'); }
    ...
    const min = ts.factory.createNumericLiteral(this.options.min);
