...
export class NumberDecoratorManager implements DecoratorManager { //FIXME : This decorator needs to be replaced with the IsInt decorator
  constructor(public options: NumberDecoratorOptions, public fieldNode?: any) { }
  ...
  createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
    ...
    const numberOptions: ts.ObjectLiteralElementLike[] = [];
    ...
    if (existingDecorator !== undefined) {
      ...
      if (existingCallExpression.arguments.length > 0) {
        if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[0])) { throw new Error('Number decorator 2nd argument must be an object literal containing the validation options'); }
        ...
        if (existingCallExpression.arguments.length > 1) {
          if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[1])) { throw new Error('Number decorator 3rd argument must be an object literal containing the validation options'); }
    ...
    const decoratorIdentifier = ts.factory.createIdentifier("IsNumber");
    const argumentsArray: ts.Expression[] = [];
    if (numberOptions.length > 0) {
      const numberOptionsObjectLiteral = ts.factory.createObjectLiteralExpression(numberOptions);
