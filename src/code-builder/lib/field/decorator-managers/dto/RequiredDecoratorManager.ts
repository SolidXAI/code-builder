...
export interface RequiredDecoratorOptions {
  isApplyRequired: boolean;
  required: boolean;
  source: ts.SourceFile;
  field: any;
}
export class RequiredDecoratorManager implements DecoratorManager {
  constructor(public options: RequiredDecoratorOptions, public fieldNode?: any) { }
  ...
  createDecorator(existingDecorator: ts.Decorator | undefined): ts.Decorator {
    ...
    if (existingDecorator !== undefined) {
      ...
      if (existingCallExpression.arguments.length > 0) {
        if (!ts.isObjectLiteralExpression(existingCallExpression.arguments[0])) { throw new Error('IsNotEmpty decorator 1st argument must be an object literal containing the validation options'); }
